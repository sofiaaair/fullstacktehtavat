...
const App = () => {
  ...
  useEffect(() => {
    blogService.getAll().then(blogitem =>
      setBlogs( blogitem.sort((a,b) => b.likes-a.likes))
    )
  }, [update])
  ...
  return (
    <div>
      ...
      {
        blogs.map(blog =>
          <Blog key={blog.id} blog={blog} updateBlog={updateBlog} user={user.name} removeBlog={removeBlog}/>)
      }
